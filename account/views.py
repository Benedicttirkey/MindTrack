...
from decimal import Decimal, InvalidOperation
from django.shortcuts import render, redirect
from .models import User, DailyRecord, ExerciseRecord, MoodRecord, SleepRecord, WorkStudyRecord
from django.utils.dateparse import parse_date, parse_datetime
import datetime
from datetime import timedelta
from django.db.models import Avg, Sum
...
def register(request):
    if request.method == 'POST':
        ...
        if password != password2:
            return render(request, 'register.html', {'err_msg': 'The passwords were inconsistent twice.'})

        # Check email not already registered
        if User.objects.filter(email=email).exists():
            return render(request, 'register.html', {'err_msg': 'This email address has been registered.'})
